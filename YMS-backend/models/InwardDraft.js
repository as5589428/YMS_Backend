...
// Define the checklist item schema (same as InwardForm)
...
const InwardDraftSchema = new mongoose.Schema({
  uniqueId: {
    type: String,
  ...
  status: {
    type: String,
    enum: ["draft", "pending", "submitted", "incomplete"],
    default: "draft"
  },
  lastUpdated: { type: Date, default: Date.now }, // Track last update time
  reason: { type: String } // Reason for moving to pending (e.g., "timeout", "appCrash")
}, { timestamps: true });

const InwardDraft = mongoose.model('InwardDraft', InwardDraftSchema);

module.exports = InwardDraft;